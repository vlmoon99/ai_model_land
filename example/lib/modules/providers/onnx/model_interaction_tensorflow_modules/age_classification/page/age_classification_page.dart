...
class AgeClassificationPage extends StatefulWidget {
  const AgeClassificationPage({super.key});

  @override
  State<AgeClassificationPage> createState() => _AgeClassificationPageState();
}

class _AgeClassificationPageState extends State<AgeClassificationPage> {
  final GlobalVM _aiModelLand = Modular.get(key: 'GlobalVM');
  final UtilsClass preprocessingClass = Modular.get<UtilsClass>();
  ...
  Future<String> runModel({required Float32List inputBytes}) async {
    ...
    return ageList[int.parse(maxKey!)];
  }
    ...
    if (result != null) {
      if (kIsWeb == true) {
        ...
        if (bytes != null && bytes.isNotEmpty) {
          ...
          final Float32List photoConvert = preprocessingClass.preprocesstest(
        ...
        if (await file.exists()) {
          ...
          final Float32List photoConvert =
              preprocessingClass.preprocessGender(resizedImage);
          setState(() {
            inputBytes = photoConvert;
          });
        } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(text: "ONNX Age Model"),
              ...
              child: Column(
                ...
                children: [
                  Align(
                    ...
                    child: Text(
                      "ONNX Model Age Classification",
                      style: Thems.textStyle,
                    ),
                  ),
                  SizedBox(height: 6),
                  Text(
                    "This model can determine the age of the person in the photo",
                    style: Thems.textStyle,
                  ),
                  Text(
                    "For interaction:",
