...
class NetworkStorage extends Repository<BaseModel> {
  ...
  Future<void> delete(String id) async {
    ...
    secureStorage.write(key: StorageKeys.network, value: jsonEncode(modals));
  }

  @override
  Future<void> deleteAll() async {
    secureStorage.delete(key: StorageKeys.network);
  }
  ...
  Future<void> save(BaseModel item) async {
    final modals =
        (jsonDecode(await secureStorage.read(key: StorageKeys.network) ?? '[]')
            ...
            .toList();
    modals.add(item);
    secureStorage.write(key: StorageKeys.network, value: jsonEncode(item));
  }

  @override
  Future<void> saveAll(List<BaseModel> items) async {
    secureStorage.write(key: StorageKeys.network, value: jsonEncode(items));
  }
